...
class FCOS(nn.Module):
    ...
    def forward(self, query, support):
        ...
        '''
        C3,C4,C5=self.backbone(query)
        all_P_query=self.fpn([C3,C4,C5])

        C3,C4,C5=self.backbone(query)
        all_P_support=self.fpn([C3,C4,C5])

        all_P = []
        for cq, cs in zip(all_P_query, all_P_support):
            h, w = cs.shape[2:]
            pooled_features = nn.AvgPool2d((h, w))(cs)
...
class FCOSDetector(nn.Module):
    ...
    def forward(self,inputs):
        ...
        if self.mode=="training":
            ...
            '''
            batch_queries, batch_support=inputs
            out=self.fcos_body(batch_queries, batch_support)
